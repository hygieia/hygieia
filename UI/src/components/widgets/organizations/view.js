(function () {
    ...
    OrganizationViewController.$inject = ['$q', '$scope','organizationRepoData', '$modal'];
    function OrganizationViewController($q, $scope, organizationRepoData, $modal) {
        var ctrl = this;

        ctrl.organizationChartOptions = {
            plugins: [
        ...
        ctrl.load = function() {
            ...
            organizationRepoData.details(params).then(function(data) {
        ...
        function showDetail(evt) {
            ...
            $modal.open({
                ...
                controllerAs: 'detail',
                templateUrl: 'components/widgets/organizations/detail.html',
                size: 'lg',
                resolve: {
                    organizations: function() {
                        return groupedorganizationData[pointIndex];
                    }
                }
            });
        }

        var groupedorganizationData = [];
        function processResponse(data, numberOfDays) {
            // get total organizations by day
            var organizations = [];
            var orgs = new Map();

            for(var x=0 ; x < data.length; x++) {
                try {
                    var dept = data[x].departmentName;
                    if (dept == null)
                        dept = "Unknown";
                    if (orgs.has(dept)) {
                        var y = orgs.get(dept);
