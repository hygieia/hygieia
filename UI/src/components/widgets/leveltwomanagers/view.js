(function () {
    ...
    angular
        ...
        .controller('LeveltwomanagerViewController', LeveltwomanagerViewController);

    LeveltwomanagerViewController.$inject = ['$q', '$scope','leveltwomanagerRepoData', '$modal'];
    function LeveltwomanagerViewController($q, $scope, leveltwomanagerRepoData, $modal) {
        var ctrl = this;

        ctrl.leveltwomanagerChartOptions = {
            plugins: [
        ...
        ctrl.load = function() {
            ...
            leveltwomanagerRepoData.details(params).then(function(data) {
        ...
        function showDetail(evt) {
            ...
            $modal.open({
                controller: 'LeveltwomanagerDetailController',
                controllerAs: 'detail',
                templateUrl: 'components/widgets/leveltwomanagers/detail.html',
                size: 'lg',
                resolve: {
                    leveltwomanagers: function() {
                        return groupedleveltwomanagerData[pointIndex];
                    }
                }
            });
        }

        var groupedleveltwomanagerData = [];
        function processResponse(data, numberOfDays) {
            // get total leveltwomanagers by day
            var leveltwomanagers = [];
            var orgs = new Map();

            for(var x=0 ; x < data.length; x++) {
                try {
                    var dept = data[x].levelTwoMgr;
                    if (dept == null)
                        dept = "Unknown";
                    if (orgs.has(dept)) {
                        var y = orgs.get(dept);
