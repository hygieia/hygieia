(function () {
    ...
    angular
        ...
        .controller('LevelonemanagerViewController', LevelonemanagerViewController);

    LevelonemanagerViewController.$inject = ['$q', '$scope','levelonemanagerRepoData', '$modal'];
    function LevelonemanagerViewController($q, $scope, levelonemanagerRepoData, $modal) {
        var ctrl = this;

        ctrl.levelonemanagerChartOptions = {
            plugins: [
        ...
        ctrl.load = function() {
            ...
            levelonemanagerRepoData.details(params).then(function(data) {
        ...
        function showDetail(evt) {
            ...
            $modal.open({
                controller: 'LevelonemanagerDetailController',
                controllerAs: 'detail',
                templateUrl: 'components/widgets/levelonemanagers/detail.html',
                size: 'lg',
                resolve: {
                    levelonemanagers: function() {
                        return groupedlevelonemanagerData[pointIndex];
                    }
                }
            });
        }

        var groupedlevelonemanagerData = [];
        function processResponse(data, numberOfDays) {
            // get total levelonemanagers by day
            var levelonemanagers = [];
            var orgs = new Map();

            for(var x=0 ; x < data.length; x++) {
                try {
                    var dept = data[x].levelOneMgr;
                    if (dept == null)
                        dept = "Unknown";
                    if (orgs.has(dept)) {
                        var y = orgs.get(dept);
