		...
		if (isEstimateTime(estimateMetricType)) {
			...
			if (!StringUtils.isEmpty(feature.getsEstimate())) {
				try {
					...
					rt = 0;
					LOG.error("Error in com.capitalone.dashboard.service.FeatureServiceImpl, could not parse estimate for '"+ feature.getsName()+ "', number '"+ feature.getsNumber()
							+ "', have estimate: "+ feature.getsEstimate());
				}
			}
		}
