...
public class FeatureServiceImpl implements FeatureService {
     ...
     *
     * @param componentRepository Repository containing components used by the UI (populated by
     *                            UI)
     * @param collectorRepository Repository containing all registered collectors
     * @param featureRepository   Repository containing all features
     */
    @Autowired
     ...
     *
     * @param componentId The ID of the related UI component that will reference
     *                    collector item content from this collector
     * @param storyNumber A back-end story ID used by a source system
    ...
    public DataResponse<List<Feature>> getFeatureEstimates(ObjectId componentId, String teamId) {
        ...
        for (Feature tempRs : relevantFeatureEstimates) {
            if (StringUtils.isEmpty(tempRs.getsEpicID())) continue;
            ...
            if (tempRs.getsEpicID().equalsIgnoreCase(lastEpicID)) {
                ...
                if (!CollectionUtils.isEmpty(relevantSuperFeatureEstimates)) {
                    Iterables.getLast(relevantSuperFeatureEstimates).setsEstimate(
    ...
    private DataResponse<List<Feature>> getEstimate(ObjectId componentId, String teamId, Status status) {
        Component component = componentRepository.findOne(componentId);
        if ((component == null) || CollectionUtils.isEmpty(component.getCollectorItems()) ||
                CollectionUtils.isEmpty(component.getCollectorItems().get(CollectorType.ScopeOwner)) ||
                (component.getCollectorItems().get(CollectorType.ScopeOwner).get(0) == null)) {
        ...
        for (Feature tempRs : storyEstimates) {
            if (StringUtils.isEmpty(tempRs.getsEstimate())) continue;
    ...
    public DataResponse<List<Feature>> getTotalEstimate(ObjectId componentId, String teamId) {
        return getEstimate(componentId, teamId, Status.TOTAL);
    }
    ...
    public DataResponse<List<Feature>> getInProgressEstimate(ObjectId componentId, String teamId) {
        return getEstimate(componentId, teamId, Status.InProgress);
    }
    ...
    public DataResponse<List<Feature>> getDoneEstimate(ObjectId componentId, String teamId) {
        return getEstimate(componentId, teamId, Status.DONE);
    }
    ...
    private String getCurrentISODateTime() {
        return DatatypeConverter.printDateTime(Calendar.getInstance(TimeZone
                .getTimeZone("UTC")));
    }
}
