...
@Component
public class SonarCollectorTask extends CollectorTask<ReportPortalCollector> {
    @SuppressWarnings({ "PMD.UnusedPrivateField", "unused" })
    private static final Log LOG = LogFactory.getLog(SonarCollectorTask.class);

    private final ReportPortalCollectorRepository sonarCollectorRepository;
    private final ReportPortalProjectRepository sonarProjectRepository;
    private final CodeQualityRepository codeQualityRepository;
    private final SonarProfileRepostory sonarProfileRepostory;
    private final ReportPortalSettings sonarSettings;
    private final ComponentRepository dbComponentRepository;

    @Autowired
    public SonarCollectorTask(TaskScheduler taskScheduler,
                              ReportPortalCollectorRepository sonarCollectorRepository,
                              ReportPortalProjectRepository sonarProjectRepository,
                              CodeQualityRepository codeQualityRepository,
                              SonarProfileRepostory sonarProfileRepostory,
                              ReportPortalSettings sonarSettings,
    ...
    public ReportPortalCollector getCollector() {
        return ReportPortalCollector.prototype(sonarSettings.getServers(), sonarSettings.getVersions(), sonarSettings.getMetrics(),sonarSettings.getNiceNames());
    }
    ...
    public void collect(ReportPortalCollector collector) {
        ...
        List<ReportPortalProject> existingProjects = sonarProjectRepository.findByCollectorIdIn(udId);
        ...
        if (!CollectionUtils.isEmpty(collector.getReportPortalServers())) {
            ...
            for (int i = 0; i < collector.getReportPortalServers().size(); i++) {
               ...
               // ReportPortalClient sonarClient = sonarClientSelector.getSonarClient(version);
                List<ReportPortalProject> projects = sonarClient.getProjects(instanceUrl);
	 ...
	 *
	 * @param collector
	 *            the {@link ReportPortalCollector}
	 */

    @SuppressWarnings("PMD.AvoidDeeplyNestedIfStmts") // agreed PMD, fixme
    private void clean(ReportPortalCollector collector, List<SonarProject> existingProjects) {
    ...
    private void deleteUnwantedJobs(List<SonarProject> latestProjects, List<SonarProject> existingProjects, ReportPortalCollector collector) {
    ...
    private void refreshData(List<SonarProject> sonarProjects, ReportPortalClient sonarClient, String metrics) {
        long start = System.currentTimeMillis();
        int count = 0;
    ...
    private void fetchQualityProfileConfigChanges(ReportPortalCollector collector,String instanceUrl,ReportPortalClient sonarClient) throws org.json.simple.parser.ParseException{
    ...
    private void addNewConfigurationChanges(ReportPortalCollector collector,JSONArray sonarProfileConfigurationChanges){
    ...
    private List<SonarProject> enabledProjects(ReportPortalCollector collector, String instanceUrl) {
        return sonarProjectRepository.findEnabledProjects(collector.getId(), instanceUrl);
    }

    private void addNewProjects(List<SonarProject> projects, List<SonarProject> existingProjects, ReportPortalCollector collector) {
        long start = System.currentTimeMillis();
        int count = 0;
    ...
    private String getNiceName(SonarProject project, ReportPortalCollector sonarCollector){
	...
	private boolean isNewProject(ReportPortalCollector collector, SonarProject application) {
