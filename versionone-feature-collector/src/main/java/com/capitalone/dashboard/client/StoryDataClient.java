...
public class StoryDataClient extends BaseClient {
    ...
    protected void updateMongoInfo(JSONArray tmpMongoDetailArray) {
        for (Object obj : tmpMongoDetailArray) {
            ...
            // sOwnersShortName
            feature.setsOwnersShortName(toCanonicalList((List<String>) dataMainObj.get("Owners.Nickname")));

            // sOwnersFullName
            feature.setsOwnersFullName(toCanonicalList((List<String>) dataMainObj.get("Owners.Name")));

            // sOwnersUsername
            feature.setsOwnersUsername(toCanonicalList((List<String>) dataMainObj.get("Owners.Username")));

            // sOwnersState
            feature.setsOwnersState(toCanonicalList((List<String>) dataMainObj.get("Owners.AssetState")));

            // sOwnersChangeDate
            feature.setsOwnersChangeDate(toCanonicalList((List<String>) dataMainObj.get("Owners.ChangeDate")));

            // sOwnersIsDeleted
            feature.setsOwnersIsDeleted(toCanonicalList((List<String>) dataMainObj.get("Owners.IsDeleted")));
